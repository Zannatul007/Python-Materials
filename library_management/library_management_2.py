...
try:
    with open('library_data.json', 'r', encoding='utf-8') as read_file:
...
def view_status():
    for id1,book_info in books_collection.items():
        if(book_info['status']=='Available'):
            print(f"ID: {id1}, Title: {book_info['title']}, Author: {book_info['author']}, Status: {book_info['status']}")
        else:
            print(f"ID: {id1}, Title: {book_info['title']}, Author: {book_info['author']}, Status: {book_info['status']} by {book_info['b_name']}")


choice = ''
choice_list = ['Add Book','Check Out Book','Return Book','View Library Status','Exit']
